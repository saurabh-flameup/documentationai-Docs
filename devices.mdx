...
The Devices API allows you to register and manage device tokens for push notifications. When users install your mobile app or enable web push, you register their device token with Flare to enable push messaging.

<Callout kind="info">**Required Permission**: `people:write` for registering devices, `people:read` for listing devices.</Callout>

## Supported Platforms

Flare supports push notifications on three platforms:
...
A device token in Flare has the following structure:
...
Device tokens have a lifecycle that Flare manages automatically:
...
<Steps>
  ...
  <Step title="Token Refresh" icon="refresh-cw">
    FCM may issue a new token. Re-register with the new token; Flare deduplicates by device.
  </Step>
  ...
  <Tab title="iOS (Swift)" icon="smartphone">
    ...
    extension AppDelegate: MessagingDelegate {
        func messaging(_ messaging: Messaging,
            ...
            // Re-register with Flare
            FlareAPI.registerDevice(token: token)
        }
    }
    ```
  </Tab>
...
<ExpandableGroup>
  ...
  <Expandable title="Handle Multiple Devices" default-open="false">
    Users may have multiple devices. Flare handles this automatically:
